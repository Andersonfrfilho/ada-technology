...
|---|---|---|
| `@` | sem registro nenhum; resolve como inexistente | so fecha com CNAME flattening — depende do provedor. Enquanto isso o site atende em `www` |

Tres itens ja foram corrigidos e viraram o estado declarado acima: o `TXT` de verificacao do Zoho,
que tinha dois valores colados num campo com aspa escapada no meio; os `MX` secundarios `mx2` (20) e
`mx3` (50), que nao existiam — sem eles, indisponibilidade do primario adia ou devolve e-mail em vez
de cair no proximo; e o `A @ 75.2.60.5`, resquicio do Netlify, que fazia o Chrome acusar
`ERR_CERT_COMMON_NAME_INVALID` porque a borda do Netlify respondia com o certificado `*.netlify.app`
para um nome que ela nao servia mais.

Remover o `A` do apex nao afeta e-mail: `MX`, `SPF`, `DKIM` e as verificacoes moram no mesmo nome,
mas sao tipos de registro independentes.
